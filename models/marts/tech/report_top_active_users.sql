...
joining_commit_count_and_pr_count as (
    ...
    select
        cc.actor_id,
        cc.actor_username,
        ifnull(cc.n_commits, 0) as n_commits,
        ifnull(pc.n_prs, 0) as n_prs,
        ifnull(cc.n_commits, 0) + ifnull(pc.n_prs, 0) as n_commits_and_prs,
...
-- Note: there are many commits that appear in the `stg_clara__events` model that don't appear in the `stg_clara__commits` model (2139 to be precise).
    ...
    -- The model `int_commits_joined` will be prefered over `stg_clara__events` as the Source-of-Truth when dealing with commit info, since it contains metadata such as SHA and commit message. 